...
export default {
  ...
  computed: {
    ...
    remarksRule () {
      return [{ key: !this.validation.adminRemarks.required, value: '入力されていません。' }]
    },
    receiptRule () {
    ...
    validation () {
      return {
        ...
        adminRemarks: { required: true },
        receipt: { required: this.detail.routeDiv === '3' ? (this.detail.receipt) : true }
      }
    },
    valid () {
  ...
  methods: {
    ...
    async setDetail () {
      ...
      for (const item of this.detailList.list) {
    ...
    onFileChange (e) {
      ...
      this.detail.receipt = e.target.files[0]
    },
    // inputタグを初期化
    resetInputFile () {
