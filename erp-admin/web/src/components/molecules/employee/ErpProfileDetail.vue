<template>
  <div>
    <div class="">
      ...
      <ErpP>採用区分：{{ profile.recruitmentClassification | recruitmentClassification }}</ErpP>
      <ErpP>在籍区分：{{ profile | classificationOfAttendance }}</ErpP>
    </div>
    <div class="">
      <ErpH3 class-name="underline">
        勤務情報
    ...
    <div class="">
      ...
      <ErpP>出身都道府県：{{ profile.birthplace | prefectural }}</ErpP>
      <ErpP>配偶者：{{ profile.spouse | spouse }}</ErpP>
      <ErpP>最終学歴：{{ profile.enducationalBackground | enducationalBackground }}</ErpP>
    </div>
    <div class="">
      <ErpH3 class-name="underline">
        連絡先
    ...
    <div class="">
      ...
      <ErpP>ログイン制御：{{ profile.loginDiv | loginDiv }}</ErpP>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    businessContent () { return this.profile.businessContent === null ? '' : this.profile.businessContent },
    note () { return this.profile.note === null ? '' : this.profile.note }
  },
  methods: {
    getNameBelongMst (id) {
