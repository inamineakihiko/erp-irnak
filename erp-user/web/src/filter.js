...
Vue.filter('transportation', (value) => {
  const transportation = consts.trafic.TRANSPORTATION_VALUE
  // eslint-disable-next-line no-unused-vars
  for (const item in transportation) {
    if (value === transportation[item].value) {
      return transportation[item].label
    }
  }
  return NUKNOWN_VALUE
})
